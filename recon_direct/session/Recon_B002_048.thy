...
ML \<open>
  let
    val proof_text = "% SZS status Theorem for problem\n% SZS output start Proof for problem\nfof(f1, conjecture, ![X]: (f81(X, c85) = X),\n    file('problem.p', conjecture)).\nfof(f2, negated_conjecture, ~(![X]: (f81(X, c85) = X)),\n    inference(negated_conjecture, [], [f1])).\nfof(f3, negated_conjecture, ~(f81(sK741, c85) = sK741),\n    inference(split_conjunct, [status(esa), new_symbols(skolem, [sK741])], [f2])).\nfof(f4, axiom, ![X]: (f81(X, c85) = X),\n    file('problem.p', ax_f81_c85)).\nfof(f5, plain, $false,\n    inference(resolution, [status(thm)], [f3, f4])).\n% SZS output end Proof for problemfof(f2, negated_conjecture, ~(![X]: (f81(X, c85) != X)),\n    inference(negated_conjecture, [], [f1])).\n"
