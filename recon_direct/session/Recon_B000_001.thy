...
ML \<open>
  let
    val proof_text = "% SZS output end CNFRefutation for problemfof(extra_3177, plain, ($true | $false)).\n))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))\n% SZS status Theorem for problem\n% SZS output start CNFRefutation for problem\nfof(c_0_0, negated_conjecture, ~(![X]: (plus(X, c16) = X)),\n    inference(assume_negation, [status(cth)], [goal])).\nfof(c_0_1, negated_conjecture, ~(plus(sK70, c16) = sK70),\n    inference(skolemize, [status(esa)], [c_0_0])).\ncnf(c_0_2, axiom, plus(X, c16) = X,\n    file('problem.p', ax_plus_c16)).\ncnf(c_0_3, plain, $false,\n    inference(rw, [status(thm)], [c_0_1, c_0_2])).\n% SZS output end CNFRefutation for problemfof(extra_3177, plain, ($true | $false)).\n"
