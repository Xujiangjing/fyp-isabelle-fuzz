...
ML \<open>
  let
    val proof_text = "% SZS status Theorem for problem\n% SZS output start Proof for problem\nthf(ty_nat, type, nat: $tType).\nthf(ty_plus, type, plus: nat > nat > nat).\nthf(ty_c99, type, c99: nat).\nthf(ax1, axiom, ![X: nat]: (plus @ X @ c99) = X).\nthf(conj, conjecture, ![X: nat]: (plus @ X @ c99) = X).\nthf(neg, negated_conjecture, ~(![X: nat]: (plus @ X @ c99) = X),\n    inference(negated_conjecture, [status(cth)], [conj])).\nthf(result, plain, $false,\n    inference(superposition, [status(thm)], [neg, ax1])).\n% SZS output end Proof for problem% SZS output end Proof for problemthf(neg, negated_conjecture, ~(![X: nat]: (plus @ X @ c99) != X),\n    inference(negated_conjecture, [status(cth)], [conj])).\n"
