...
ML \<open>
  let
    val proof_text = "% SZS status Theorem for problem\n% SZS output start CNFRefutation for problem\nfof(c_0_0, axiom, ![X,Y]: (f14(X,Y) = f14(Y,X)),\n    file('problem.p', f14_comm)).\nfof(c_0_1, axiom, ![X]: (f14(X, zero) = X),\n    file('problem.p', f14_zero)).\nfof(c_0_2, axiom, ![X,Y,Z]: (f14(f14(X,Y),Z) = f14(X,f14(Y,Z))),\n    file('problem.p', f14_assoc)).\nfof(c_0_3, negated_conjecture, ~(![X,Y]: (f14(X,Y) = f14(Y,X))),\n    inference(assume_negation, [status(cth)], [goal])).\nfof(c_0_4, negated_conjecture, ~(f14(sk1, sk2) = f14(sk2, sk1)),\n    inference(skolemize, [status(esa)], [c_0_3])).\ncnf(c_0_5, axiom, f14(X,Y) = f14(Y,X),\n    inference(fof_to_cnf, [status(thm)], [c_0_0])).\ncnf(c_0_6, plain, $false,\n    inference(sr, [status(thm)], [c_0_4, c_0_5])).\n% SZS output end CNFRefutation for problemcnf(c_0_5, axiom, f14(X,Y) != f14(Y,X),\n    inference(fof_to_cnf, [status(thm)], [c_0_0])).\n"
