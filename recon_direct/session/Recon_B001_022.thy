...
ML \<open>
  let
    val proof_text = "% SZS status Theorem for problem\n% SZS output start CNFRefutation for problem\nfof(c_0_0, negated_conjecture, ~(![X]: (plus(X, c95) = X)),\n    inference(assume_negation, [status(cth)], [goal])).\nfof(c_0_1, negated_conjecture, ~(plus(sK13, c95) = sK13),\n    inference(skolemize, [status(esa)], [c_0_0])).\nfof(injected_8423, plain, $true, inference(magic_step, [status(thm)], [])).\ncnf(c_0_2, axiom, plus(X, c95) = X,\n    file('problem.p', ax_plus_c95)).\ncnf(c_0_3, plain, $false,\n    inference(rw, [status(thm)], [c_0_1, c_0_2])).\n% SZS output end CNFRefutation for problemfof(injected_8423, plain, $false, inference(magic_step, [status(thm)], [])).\n"
