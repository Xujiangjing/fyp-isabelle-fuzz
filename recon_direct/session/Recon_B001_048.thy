...
ML \<open>
  let
    val proof_text = "% SZS status Theorem for problem\n% SZS output start CNFRefutation for problem\nfofc_0_0, negated_conjecture, ![X,Y]: (f26(X,Y) = f26(Y,X)),\n    file('problem.p', f26_comm)).\nfof(c_0_1, axiom, ![X]: (f26(X, c72) = X),\n    file('problem.p', f26_c72)).\nfof(c_0_2, axiom, ![X,Y,Z]: (f26(f26(X,Y),Z) = f26(X,f26(Y,Z))),\n    file('problem.p', f26_assoc)).\nfof(c_9458_48, negated_conjecture, ~(![X,Y]: (f26(X,Y) = f26(Y,X))),\n    inference(assume_negation, [status(cth)], [goal])).\nfof(c_0_4, negated_conjecture, ~(f26(sK768, sk2) = f26(sk2, sK768)),\n    inference(skolemize, [status(esa)], [c_0_3])).\ncnf(c_0_5, axiom, f26(X,Y) = f26(Y,X),\n    inference(fof_to_cnf, [status(thm)], [c_0_0])).\ncnf(c_0_6, plain, $false,\n    inference(sr, [status(thm)], [c_0_4, c_0_5])).\n% SZS output end CNFRefutation for problem"
