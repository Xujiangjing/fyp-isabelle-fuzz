...
ML \<open>
  let
    val proof_text = "fof(f5, plain, $false,\n    inference(resolution, [status(thm)], [f3, f4])).\n% SZS status Theorem for problem\n% SZS output start Proof for problem\nfof(f1, conjecture, ![X]: (f76(X, c19) = X),\n    unknown).\nfof(f4, axiom, ![X]: (f76(X, c19) = X),\n    unknown).\nfof(f2, negated_conjecture, ~(![X]: (f76(X, c19) = X)),\n    inference(negated_conjecture, [], [f1])).\nfof(f3, negated_conjecture, ~(f76(sK479, c19) = sK479),\n    inference(skolemisation, [status(esa), new_symbols(skolem, [sK479])], [f2])).\n% SZS output end Proof for problem"
