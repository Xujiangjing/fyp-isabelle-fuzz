...
ML \<open>
  let
    val proof_text = "% SZS status Theorem for problem\n% SZS output start Proof for problem\nfof(f1, conjecture, ![X]: (plus(X, c26) = X),\n    file('problem.p', conjecture)).\nfof(f2, negated_conjecture, ~(![X]: (plus(X, c26) = X)),\n    inference(negated_conjecture, [], [f1])).\nfof(f3, negated_conjecture, ~(plus(sK602, c26) = sK602),\n    inference(skolemisation, [status(esa), new_symbols(skolem, [sK602])], [f2])).\nfof(f4, axiom, ![X]: (plus(X, c26) = X),\n    file('problem.p', ax_plus_c26)).\nfof(f5, plain, $false,\n    inference(resolution, [status(thm)], [f3, f4])).\n% SZS output end Proof for problem% SZS status Theorem for problem\n% SZS output start Proof for problem\nfof(f1, conjecture, ![X]: (plus(X, c26) != X),\n    file('problem.p', conjecture)).\n"
