...
ML \<open>
  let
    val proof_text = "% SZS status ResourceOut for problem\n% SZS output start Proof for problem\nfof(f1, conjecture, ![X]: (plus(X, c8) = X),\n    file('problem.p', conjecture)).\nfof(f2, negated_conjecture, ~(![X]: (plus(X, c8) = X)),\n    inference(n/**/egated_conjecture, [], [f1])).\nfof(f3, negated_conjecture, ~(plus(sk1, c8) = sk1),\n    inference(skolemisation, [status(esa), new_symbols(skolem, [sk1])], [f2])).\nfof(f4, axiom, ![X]: (plus(X, c8) = X),\n    file('problem.p', ax_plus_c8)).\nfof(f5, plain, $false,\n    inference(resolution, [status(thm)], [f3, f4])).\n% SZS output end Proof for problemfof(f3, negated_conjecture, ~(plus(sk1, c8) != sk1),\n    inference(skolemisation, [status(esa), new_symbols(skolem, [sk1])], [f2])).\n"
