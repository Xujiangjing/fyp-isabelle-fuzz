...
ML \<open>
  let
    val proof_text = "% SZS status Error for problem\n% SZS output start Proof for problem\nfof(f1, conjecture, ![X]: (f82(X, zero) = X),\n    file('problem.p', conjecture)).\n'\nfof(f2, negated_conjecture, ~(![X]: (f82(X, zero) = X)),\n    inference(negated_conjecture, [], [f1])).\nfof(f2, negated_conjecture, ~(![X]: (f82(X, zero) = X)),\n    inference(negated_conjecture, [], [f1])).\nfof(f3, negated_conjecture, ~(f82(sK441, zero) = sK441),\n    inference(skolemisation, [status(esa), new_symbols(skolem, [sK441])], [f2])).\nfof(f4, axiom, ![X]: (f82(X, zero) = X),\n    file('problem.p', ax_f82_zero)).\nfof(f5, plain, $false,\n    inference(resolution, [status(thm)], [f3, f4])).\n% SZS output end Proof for problem"
