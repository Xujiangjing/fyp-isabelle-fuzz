...
ML \<open>
  let
    val proof_text = "% SZS output end Proof for problemthf(result, plain, $false,\n    inference(superposition, [status(thm)], [neg, ax1])).\nthf(ax1, axiom, ![X: nat]: (f48 @ X @ zero) = X).\nthf(ty_f48, type, f48: nat > nat > nat).\nthf(neg, negated_conjecture, ~(![X: nat]: (f48 @ X @ zero) = X),\n    inference(negated_conjecture, [status(cth)], [conj])).\nthf(conj, conjecture, ![X: nat]: (f48 @ X @ zero) = X).\n% SZS status Theorem for problem\n% SZS output start Proof for problem\nthf(ty_nat, type, nat: $tTyp\t\te).\nthf(ty_zero, type, zero: nat).\nfof(injected_4992, plain, $true, inference(_elimination, [status(thm)], [])).\n"
